...
public class SimpleRunnableTask implements Runnable{
	...
	// This overloaded function is for display the data array in a random order along with the thread name
	public void displayRandomData(String threadName) {
		try {
			// Check if thread name is word1, if yes then suspend thread for 5 seconds
			if(threadName.equals("word1")) {
				System.out.println("Current thread named "+ threadName +" is suspended for 5 seconds");
				Thread.sleep(5000);
			}
			...
			System.out.println(Arrays.toString(data) + " - " + threadName);
		}catch(Exception e) {
			e.printStackTrace();
		}		
	}
	 ...
	 * If current thread name is word1 then suspend the thread for 5 seconds
	 */
	@Override
	public void run() {
		...
		if(currentThread.getName().equals("text")) {
			...
			displayRandomData(currentThread.getName());
		}
	}
